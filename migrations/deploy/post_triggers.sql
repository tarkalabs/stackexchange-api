...
CREATE FUNCTION stackdump_private.post_update_trigger() RETURNS TRIGGER AS $$
    ...
    BEGIN
        ...
        IF new.title != old.title THEN
            INSERT INTO stackdump.postHistory(postHistoryTypeId, postId, revisionGUID, userId, text)
                VALUES (4, new.id, extensions.uuid_generate_v4(), new.ownerUserId, new.title);
        END IF;
        IF new.body != old.body THEN
            INSERT INTO stackdump.postHistory(postHistoryTypeId, postId, revisionGUID, userId, text)
                VALUES (5, new.id, extensions.uuid_generate_v4(), new.ownerUserId, new.body);
        END IF;
        IF new.tags != old.tags THEN
            ...
            IF array_upper(new.tags, 1) >  1 THEN 
                ...
                END LOOP;
                INSERT INTO stackdump.postHistory(postHistoryTypeId, postId, revisionGUID, userId, text)
                    VALUES (6, new.id, extensions.uuid_generate_v4(), new.ownerUserId, new.tags);
            END IF;
        END IF;
        RESET ROLE;
        RETURN new;
